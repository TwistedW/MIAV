...
class Compose(object):
    ...
    def __call__(self, img, mask_A, mask_V):
        assert img.size == mask_A.size and img.size == mask_V.size
        for t in self.transforms:
            img, mask_A, mask_V = t(img, mask_A, mask_V)
        return img, mask_A, mask_V


class RandomHorizontallyFlip(object):
    def __call__(self, img, mask_A, mask_V):
        if random.random() < 0.5:
            return img.transpose(Image.FLIP_LEFT_RIGHT), mask_A.transpose(Image.FLIP_LEFT_RIGHT), mask_V.transpose(Image.FLIP_LEFT_RIGHT)
        return img, mask_A, mask_V
...
class JointResize(object):
    ...
    def __call__(self, img, mask_A, mask_V):
        img = img.resize(self.size, resample=Image.BILINEAR)
        mask_A = mask_A.resize(self.size, resample=Image.NEAREST)
        mask_V = mask_V.resize(self.size, resample=Image.NEAREST)
        return img, mask_A, mask_V
...
class RandomRotate(object):
    ...
    def __call__(self, img, mask_A, mask_V):
        rotate_degree = random.random() * 2 * self.degree - self.degree
        return img.rotate(rotate_degree, Image.BILINEAR), mask_A.rotate(rotate_degree, Image.NEAREST), mask_V.rotate(rotate_degree, Image.NEAREST)
...
class RandomScaleCrop(object):
    ...
    def __call__(self, img, mask_A, mask_V):
        ...
        o_size = random.randint(int(self.input_size * 1), int(self.input_size * self.scale_factor))
        img = img.resize((o_size, o_size), resample=Image.BILINEAR)
        mask_A = mask_A.resize((o_size, o_size), resample=Image.NEAREST)  # mask的放缩使用的是近邻差值
        mask_V = mask_V.resize((o_size, o_size), resample=Image.NEAREST)

        # random crop input_size
        x1 = random.randint(0, o_size - self.input_size)
        y1 = random.randint(0, o_size - self.input_size)
        img = img.crop((x1, y1, x1 + self.input_size, y1 + self.input_size))
        mask_A = mask_A.crop((x1, y1, x1 + self.input_size, y1 + self.input_size))
        mask_V = mask_V.crop((x1, y1, x1 + self.input_size, y1 + self.input_size))
...
class ScaleCenterCrop(object):
    ...
    def __call__(self, img, mask_A, mask_V):
        ...
        if w > h:
            ...
            oh = int(1.0 * h * ow / w)
        img = img.resize((ow, oh), resample=Image.BILINEAR)
        mask_A = mask_A.resize((ow, oh), resample=Image.NEAREST)
        mask_V = mask_V.resize((ow, oh), resample=Image.NEAREST)
